...
describe('config', function(){
    ...
    describe('#read()', function(){
        ...
        it('should callback with an object and no error but with lint', function(done){
            temp.mkdir('nog-cli-test-', function(err, result){
                config.read(result, function(err, result){
                    expect(err).to.be.null;
                    expect(result).to.be.an('object');
                    expect(result.__lint).to.include.keys('_invalid_json');
                    expect(result.title).to.equal(defaults.title);
                    done();
                });
            });
        });
        it('should callback with an object and no error but lint if the user site.json contains invalid json', function(done){
            temp.mkdir('nog-cli-test-', function(err, result){
                fs.outputFile(path.join(result, '_cfg', 'site.json'), 'fooo', function(){
                    config.read(result, function(err, result){
                        expect(err).to.be.null;
                        expect(result).to.be.an('object');
                        expect(result.__lint).to.include.keys('_invalid_json');
        ...
        it('should callback with an object and no error but lint if the user site.json resolves to something other than an object', function(done){
            temp.mkdir('nog-cli-test-', function(err, result){
                fs.outputFile(path.join(result, '_cfg', 'site.json'), JSON.stringify(['foo']), function(){
                    config.read(result, function(err, result){
                        expect(err).to.be.null;
                        expect(result).to.be.an('object');
                        expect(result.__lint).to.include.keys('_invalid_json');
        ...
        it('should callback with an object and no error and no lint if the user site.json resolves to an object', function(done){
            temp.mkdir('nog-cli-test-', function(err, result){
                fs.outputFile(path.join(result, '_cfg', 'site.json'), JSON.stringify({title: 'foo'}), function(){
                    config.read(result, function(err, result){
                        expect(err).to.be.null;
                        expect(result).to.be.an('object');
                        expect(result.__lint).not.to.include.keys('_invalid_json');
    ...
    describe('#merge_config()', function(){
        ...
        _.each(['title', 'tagline', 'archives_tag_title_format', 'archives_title'], function(key){
            it('should set config.' + key +  ' to the default if it is not passed, with no lint', function () {
                ...
                var lint;
                lint = {};
                cfg = config.merge_config(defaults, {}, lint);
            ...
            it('should set config.' + key +  ' to the default if the passed value is not a string, with lint', function () {
                ...
                _.each(tests, function(val){
                    ...
                    my_cfg = {};
                    my_cfg[key] = val;
                    cfg = config.merge_config(defaults, my_cfg, lint);
                    expect(cfg[key]).to.equal(defaults[key]);
        ...
        it('should set config.archives_directory to "" if an empty string is passed passed, with no lint', function () {
            ...
            var lint;
            lint = {};
            cfg = config.merge_config(defaults, {archives_directory: '   '}, lint);
